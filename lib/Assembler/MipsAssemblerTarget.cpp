...
MemoryValue RType(const Entry& entry) {
...
MemoryValue IType(const Entry& entry) {
    MemoryValue instruction {};
...
MemoryValue JType(const Entry& entry) {
    MemoryValue instruction {};
    instruction.data.u32 = OpCode << 26U;
    instruction.size = 4;
    ...
    MemoryValue ParseJALR(const Entry& entry) {
    try {
...
MemoryValue ParseCOPz(const Entry& entry) {
...
MemoryValue ThrowInvalidCoprocessor(const Entry& entry) {
    throw "Instruction not supported by coprocessor: " + entry.operation.value();
}

typedef MemoryValue (*ParseFunc)(const Entry&);
...
class MipsOperationHandler : public AssemblerOperationHandler {
    bool Handle(const Entry& entry, AssemblyState& state) override {
        ...
        if (handler_kv != instructions_fn.end()) {
            ...
            state.psect.code_data[state.result->counter.code] = std::move(instruction);
